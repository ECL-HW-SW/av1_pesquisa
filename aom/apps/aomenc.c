...
const arg_def_t *global_args[] = {
  ...
  &g_av1_codec_arg_defs.disable_prune_partitions_before_search,
  &g_av1_codec_arg_defs.disable_prune_partitions_after_split,
  &g_av1_codec_arg_defs.disable_prune_4_way_partition_search,
  &g_av1_codec_arg_defs.disable_av1_prune_ab_partitions,
  &g_av1_codec_arg_defs.disable_av1_ml_prune_4_partition,
  &g_av1_codec_arg_defs.disable_prune_4_partition_using_split_info,
  &g_av1_codec_arg_defs.disable_av1_ml_prune_rect_partition,
  &g_av1_codec_arg_defs.disable_prune_partitions_after_none,
#if CONFIG_WEBM_IO
...
int main(int argc, const char **argv_) {
  ...
  FILE *feat_prune_rect = fopen("output_files/get_prune_rect.csv", "w");
  FILE *feat_early_term_none = fopen("output_files/early_term_none.csv", "w");
  FILE *feat_early_term_after_split =
      fopen("output_files/get_early_term_after_split.csv", "w");
  FILE *feat_prune_rect_ml = fopen("output_files/get_prune_rect_ML.csv", "w");
  FILE *feat_prune_ab_partition =
      fopen("output_files/get_prune_ab_partition.csv", "w");
  FILE *feat_prune_4_partition =
      fopen("output_files/get_prune_4_partition.csv", "w");

  fprintf(feat_based_split,
          "(nosplit) ;(nosplit) ;(split1) ;(split1) ;(split2) ;(split2) "
          ";(split3) ;(split3) ;(split4) ;(split4) ;(dc_q * dc_q) / 256.0f "
          ";has_above ;mi_size_wide_log2(above) ;mi_size_high_log2(above) "
          ";has_left ;mi_size_wide_log2(left) ;mi_size_high_log2(left) "
          ";frame_num ;mi_row ;mi_col ;bsize ;particionado \n");
  fclose(feat_based_split);

  fprintf(
      feat_prune_rect,
      "(nosplit) ;(nosplit) ;(split1) ;(split1) ;(split2) ;(split2) ;(split3) "
      ";(split3) ;(split4) ;(split4) ;(rect1) ;(rect2) ;(rect3) ;(rect4) "
  ...
  fprintf(feat_early_term_none,
          "(nosplit) ;(nosplit) ;(split1) ;(split1) ;(split2) ;(split2) "
          ";(split3) ;(split3) ;(split4) ;(split4) ;(rect1) ;(rect2) ;(rect3) "
          ";(rect4) ;(rect5) ;(rect6) ;(rect7) ;(rect8) ;(dc_q * dc_q) / "
          "256.0f ;has_above ;mi_size_wide_log2(above) "
          ";mi_size_high_log2(above) ;has_left ;mi_size_wide_log2(left) "
          ";mi_size_high_log2(left) ;rate; dist; rdcost ;frame_num ;mi_row "
          ";mi_col ;bsize ;particionado \n");
  fclose(feat_early_term_none);

  fprintf(feat_early_term_after_split,
          "(dc_q / 4.0f) ;(best_rd / bs / bs / 1024.0f) ;rd_valid(none) "
          ";rd_ratio(rd / best_rd)(none) ;rd_valid(split) ;rd_ratio(rd / "
          "best_rd)(split) ;rd_valid(sub1) ;rd_ratio(rd / best_rd)(sub1) "
          ";min_bw(sub1) ;min_bh(sub1) ;rd_valid(sub2) ;rd_ratio(rd / "
          "best_rd)(sub2) ;min_bw(sub2) ;min_bh(sub2) ;rd_valid(sub3) "
          ";rd_ratio(rd / best_rd)(sub3) ;min_bw(sub3) ;min_bh(sub3) "
  ...
  fprintf(feat_prune_ab_partition,
          ...
          ";rd_ratio(sub4) ;rd_ratio(sub5) ;rd_ratio(sub6) ;rd_ratio(sub7) "
          ";rd_ratio(sub8) ;frame_num ;mi_row ;mi_col ;bsize ;particionado \n");
  fclose(feat_prune_ab_partition);

  fprintf(feat_prune_4_partition,
          "part_ctx ;pb_source_variance(get_unsigned_bits) ;rd_ratio(sub1) "
          ";rd_ratio(sub2) ;rd_ratio(sub3) "
          ";rd_ratio(sub4) ;rd_ratio(sub5) ;rd_ratio(sub6) ;rd_ratio(sub7) "
          ";rd_ratio(sub8) ;var_ratio(horz4)(sub1) ;var_ratio(horz4)(sub2) "
  ...
  FILE *feat_based_split_norm = fopen("output_files/based_split_norm.csv", "w");
  FILE *feat_prune_rect_norm =
      fopen("output_files/get_prune_rect_norm.csv", "w");
  FILE *feat_early_term_none_norm =
      fopen("output_files/early_term_none_norm.csv", "w");
  FILE *feat_early_term_after_split_norm =
      fopen("output_files/get_early_term_after_split_norm.csv", "w");
  FILE *feat_prune_rect_ml_norm =
      fopen("output_files/get_prune_rect_ML_norm.csv", "w");
  FILE *feat_prune_ab_partition_norm =
      fopen("output_files/get_prune_ab_partition_norm.csv", "w");
  FILE *feat_prune_4_partition_norm =
      fopen("output_files/get_prune_4_partition_norm.csv", "w");

  fprintf(feat_based_split_norm,
          "(nosplit) ;(nosplit) ;(split1) ;(split1) ;(split2) ;(split2) "
          ";(split3) ;(split3) ;(split4) ;(split4) ;(dc_q * dc_q) / 256.0f "
          ";has_above ;mi_size_wide_log2(above) ;mi_size_high_log2(above) "
          ";has_left ;mi_size_wide_log2(left) ;mi_size_high_log2(left) "
  ...
  fprintf(feat_early_term_none_norm,
          "(nosplit) ;(nosplit) ;(split1) ;(split1) ;(split2) ;(split2) "
          ";(split3) ;(split3) ;(split4) ;(split4) ;(rect1) ;(rect2) ;(rect3) "
          ";(rect4) ;(rect5) ;(rect6) ;(rect7) ;(rect8) ;(dc_q * dc_q) / "
          "256.0f ;has_above ;mi_size_wide_log2(above) "
  ...
  fprintf(feat_early_term_after_split_norm,
          "(dc_q / 4.0f) ;(best_rd / bs / bs / 1024.0f) ;rd_valid(none) "
          ";rd_ratio(rd / best_rd)(none) ;rd_valid(split) ;rd_ratio(rd / "
          "best_rd)(split) ;rd_valid(sub1) ;rd_ratio(rd / best_rd)(sub1) "
          ";min_bw(sub1) ;min_bh(sub1) ;rd_valid(sub2) ;rd_ratio(rd / "
  ...
  fprintf(feat_prune_rect_ml_norm,
          "none_rd / best_rd ;split1_rd / best_rd ;split2_rd / best_rd "
          ";split3_rd / best_rd ;split4_rd / best_rd ;frame_num ;mi_row "
          ";mi_col ;bsize ;particionado_hor ;particionado_ver \n");
  fclose(feat_prune_rect_ml_norm);

  fprintf(feat_prune_ab_partition_norm,
          "part_ctx ;var_ctx ;rd_ratio(sub1) ;rd_ratio(sub2) ;rd_ratio(sub3) "
          ";rd_ratio(sub4) ;rd_ratio(sub5) ;rd_ratio(sub6) ;rd_ratio(sub7) "
          ";rd_ratio(sub8) ;frame_num ;mi_row ;mi_col ;bsize ;particionado \n");
  fclose(feat_prune_ab_partition_norm);

  fprintf(feat_prune_4_partition_norm,
          "part_ctx ;pb_source_variance(get_unsigned_bits) ;rd_ratio(sub1) "
          ";rd_ratio(sub2) ;rd_ratio(sub3) "
          ";rd_ratio(sub4) ;rd_ratio(sub5) ;rd_ratio(sub6) ;rd_ratio(sub7) "
          ";rd_ratio(sub8) ;var_ratio(horz4)(sub1) ;var_ratio(horz4)(sub2) "
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    //@grellert - Alan
    char *blocksizes_name[] = { "BLOCK_4X4",     "BLOCK_4X8",    "BLOCK_8X4",
                                "BLOCK_8X8",     "BLOCK_8X16",   "BLOCK_16X8",
                                "BLOCK_16X16",   "BLOCK_16X32",  "BLOCK_32X16",
                                "BLOCK_32X32",   "BLOCK_32X64",  "BLOCK_64X32",
                                "BLOCK_64X64",   "BLOCK_64X128", "BLOCK_128X64",
                                "BLOCK_128X128", "BLOCK_4X16",   "BLOCK_16X4",
                                "BLOCK_8X32",    "BLOCK_32X8",   "BLOCK_16X64",
                                "BLOCK_64X16" };
    ...
    fprintf(arq_partition, "Partition times for pass %d:\n",
            global.ecl_timers.pass);
    for (i = 0; i < 22; i++) {
      printf("%dx %s %g\n", global.ecl_timers.block_counter[i],
             blocksizes_name[i], global.ecl_timers.block_timer_acc[i]);
