...
void av1_simple_motion_search_based_split(AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  */
  ...
  FILE *feat_based_split_norm = fopen("output_files/based_split_norm.csv", "a");

  for (int idx = 0; idx < FEATURE_SIZE_SMS_SPLIT; idx++) {
    fprintf(feat_based_split, "%f;", features[idx]);
    features[idx] = (features[idx] - ml_mean[idx]) / ml_std[idx];
    fprintf(feat_based_split_norm, "%f;", features[idx]);
  }

  fprintf(feat_based_split, "%d;%d;%d;%d;\n", cm->current_frame.frame_number,
          mi_row, mi_col, bsize);

  fprintf(feat_based_split_norm, "%d;%d;%d;%d;\n",
...
void av1_simple_motion_search_prune_rect(AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int f_idx = 0; f_idx < FEATURE_SIZE_SMS_PRUNE_PART; f_idx++) {
    ...
    fprintf(feat_prune_rect_norm, "%f;", features[f_idx]);
  }

  fprintf(feat_prune_rect, "%d;%d;%d;%d;\n", cm->current_frame.frame_number,
          mi_row, mi_col, bsize);
  fprintf(feat_prune_rect_norm, "%d;%d;%d;%d;\n",
          cm->current_frame.frame_number, mi_row, mi_col, bsize);
...
void av1_simple_motion_search_early_term_none(
  ...
  const int mi_row = blk_params->mi_row, mi_col = blk_params->mi_col;
  const BLOCK_SIZE bsize = blk_params->bsize;
  const AV1_COMMON *const cm = &cpi->common;  //@icaro
  ...
  if (ml_model) {
    ...
    for (f_idx = 0; f_idx < FEATURE_SIZE_SMS_TERM_NONE; f_idx++) {
      ...
      fprintf(feat_early_term_none_norm, "%f;", features[f_idx]);
    }
    ...
    fprintf(feat_early_term_none_norm, "%d;%d;%d;%d;\n",
            cm->current_frame.frame_number, mi_row, mi_col, bsize);
...
void av1_ml_early_term_after_split(AV1_COMP *const cpi, MACROBLOCK *const x,
  ...
  //     fopen("aom/output_files/get_early_term_after_split_norm.csv", "a");
